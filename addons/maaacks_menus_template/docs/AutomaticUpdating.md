...
If an update is available, then the option to update will be listed at **Project > Tools > Update Plugins... > Maaack's Menus Template to v...**.
...
To disable automatic updates by plugin, go into the **Project > Project Settings > General** tab, and remove the entries in the `plugin_updater/plugins` setting (ex. `"res://addons/maaacks_menus_template/"`). You can then close the window.
